...
@AutoService(Processor.class) public final class HolderBuilderProcessor extends AbstractProcessor {
  ...
  @NonNull private List<? extends VariableElement> findConstructorParameters(TypeElement type) {
  ...
  @NonNull private ExecutableElement findConstructor(TypeElement type) {
  ...
  @NonNull private String generatedSubclassName(TypeElement type, int depth) {
    return generatedClassName(type, Strings.repeat("$", depth) + POSTFIX_BUILDER);
  }

  @NonNull private String generatedClassName(final TypeElement type, final String postfix) {
